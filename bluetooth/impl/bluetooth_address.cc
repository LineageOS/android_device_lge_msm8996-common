...
bool BluetoothAddress::get_local_address(uint8_t* local_addr) {
   ...
   ALOGD("ETMSYS: Entering %s", __func__);
   ALOGD("ETMSYS: kStringLength %zu", kStringLength);
   ALOGD("ETMSYS: kNvPathSize %zu", kNvPathSize);

// check persist first
  ALOGD("ETMSYS: %s, opening: %s", __func__,  kNvPath);
  int addr_fd = open(kNvPath, O_RDONLY);
  ALOGD("ETMSYS: addr_fd = %d", addr_fd);
  if (addr_fd != -1) {
    char address[kNvPathSize + 1] = {0};
    ALOGD("ETMSYS: opened  %s ok", kNvPath);

    int bytes_read = read(addr_fd, address, kNvPathSize);
    ALOGD("ETMSYS: bytes_read = %d", bytes_read);
...
// misc partition is blank, generate random
  ...
  // Last two bits of the first octet are special
     ...
     close(writefd);
     ALOGD("ETMSYS: random true: %s", macbuf);
