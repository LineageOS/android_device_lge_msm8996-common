...
echo "sgdisk --delete=${SYSPARTNUM} --delete=${NEXT} --new=${SYSPARTNUM}:${SYSSTART}:${SYSEND} --change-name=${SYSPARTNUM}:system --print ${DISK}"
echo "**reboot recovery**, then resize2fs"
echo "resize2fs /dev/block/bootdevice/by-name/system"

echo "To revert /vendor back: " > /data/local/tmp/vendor.recover.txt
echo "sgdisk --delete=${SYSPARTNUM} --delete=${NEXT} --new=${SYSPARTNUM}:${SYSSTART}:${SYSEND} --change-name=${SYSPARTNUM}:system --print ${DISK}" >> /data/local/tmp/vendor.recover.txt
echo "**reboot recovery**, then resize2fs" >> /data/local/tmp/vendor.recover.txt
echo "resize2fs /dev/block/bootdevice/by-name/system" >> /data/local/tmp/vendor.recover.txt
