...
setup_vendor "$PLATFORM_COMMON" "$VENDOR" "$CM_ROOT" true $clean_system
...
setup_vendor "$DEVICE_COMMON" "$VENDOR" "$CM_ROOT" true $clean_system

extract "$MY_DIR"/../$DEVICE_COMMON/proprietary-files.txt "$SRC" "$SECTION"

# Reinitialize the helper for device
setup_vendor "$DEVICE" "$VENDOR" "$CM_ROOT" false $clean_system
